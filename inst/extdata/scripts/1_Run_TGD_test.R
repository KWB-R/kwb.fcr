...
folder <- "Y:/AUFTRAEGE/_Auftraege_abgeschlossen/UFO-Phorwärts/Data-Work packages/Risikomodell/R-Skript/"
...
################################ Daten Laden #################################
##############################################################################
Sub <- read_excel(path = paste0(folder, "Substance_Sheet_test.xlsx"), 
                  sheet = "Input", col_names = TRUE, na = "NA")

Env <- read_excel(path = paste0(folder, "Environment_Sheet_test.xlsx"), 
                  sheet = "Input", col_names = TRUE, na = "NA")
...
if(FALSE){
  ...
  result <- RunTGD(runs = 100, years = 10, Substance = Substance, K_d = K_d,
